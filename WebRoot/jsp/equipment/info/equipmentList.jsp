...
function deleteData(id){
	if(confirm("是否删除？")){
		$.ajax({
			type : 'post',
			url : "<%=basePath%>/equipment/delete.action?random="+Math.random(),
			dataType:'json',
			cache : false,
			data:{
				id:id
...
<iframe id="iframe" style="display:none;"></iframe>
	<div class="main"><!--Ctrl-->
		...
		<div class="filter">
			...
			<s:form action="showList" name="equipment" namespace="/equipment" method="post" id="form">
			...
			<input type="hidden" name="pageNo" id="pageNo" value="<s:property value='#request.page.currentPageNo'/>"/>
				<table width="100%" border="0" cellspacing="0" cellpadding="0">
					<tr id="search">
						...
						<td>
							<input type="text" name="equipmentId" id="textfield" class="input_large" value="<s:property value='equipmentInfo.equipmentId'/>" >
						</td>
						<td class="t_r">设备名称</td>
						<td>
							<input type="text" name="equipmentName" id="textfield" class="input_large" value="<s:property value='equipmentInfo.equipmentName'/>" >
						</td>
						<td class="t_r">产地</td>
						<td>
							<input type="text" name="provenance" id="textfield" class="input_large" value="<s:property value='equipmentInfo.provenance'/>" >
						</td>
						<!-- 
						...
						<td>
							<select name="manufacturer" style="width:200px;">
								...
								<s:iterator value="#request.supplierList" id="supplier">
									<s:if test="#request.manufacturer==#supplier.id">
										<option value="<s:property value="#supplier.id"/>" selected="selected"><s:property value="#supplier.name"/></option>
									</s:if>
									<s:else>
										<option value="<s:property value="#supplier.id"/>"><s:property value="#supplier.name"/></option>
									</s:else>
								</s:iterator>
							</select>
						</td>
					...
					<tr id="search">
						...
						<td>
							<input type="text" name="productModel" id="textfield" class="input_large" value="<s:property value='equipmentInfo.productModel'/>" >
						</td>
					</tr>
					<tr>
						<td></td>
			...
			<tbody>
				...
				<s:iterator value="#request.page.result" id="equipment" status="st">
					<tr>
						...
						<td class="t_c"><s:property value="#equipment.assetId"/></td>
						<td class="t_c"><s:property value="#equipment.equipmentName"/></td>						
						<td class="t_c">
							<s:property value="#request.supplierNameList[#st.index]"/>
						</td>
						<td class="t_c"><s:property value="#equipment.provenance"/></td>
						<td class="t_c"><s:property value="#equipment.productModel"/></td>
						<td class="t_c">
							<a href="showView.action?id=<s:property value="#equipment.id"/>" style="float:inline;" target="_blank">查看</a>
						</td>
						<td class="t_c">
							<a href="javascript:void(0);" onclick="showEditPage('<s:property value="#equipment.id"/>');" style="float:inline;">编辑</a>
						</td>
						<td class="t_c">
							<a href="#" style="float:inline;" onclick="deleteData('<s:property value="#equipment.id"/>');">删除</a>
						</td>
					</tr>
				</s:iterator>
			</tbody>
