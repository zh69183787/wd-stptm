...
<%@page import="java.util.Properties"%><html lang="en">
<head>
<meta charset="utf-8" />
<title>资产新增</title>
...
//错误信息是否存在
$(function(){
	var failInfo =$("#failInfo").val();
	if(failInfo!=null && ""!=failInfo)
		alert(failInfo);
		
		

	var zlh = $("input[name=assetId]").val();
	if(zlh!=null && ""!=zlh){
		zlh = zlh.substr(4,1);
		$("select[name=zlh]").children().each(function(){
			if($(this).val()==zlh){
				$(this).attr("selected",true);
			} 
		});
	}
	$("input[name=assetId]").focus().select();
});
</script>
...
function checkForm(){
	...
	if(value=='-1'){
		...
		valueAsset = $("input[name=assetId]").val();		//资产编号
		var xlh;
		...
		if(valueAsset.substr(11,2)==dlh && valueAsset.substr(13,2)==zlh && valueAsset.substr(15,2)=='00'){
			return true;
		}else if(valueAsset.substr(11,2)==dlh && valueAsset.substr(13,2)==zlh && valueAsset.substr(15,2)!='00'){
			//alert("资产编号第16位和第17位数字不为00，与小类号不一致，请检查资产编号是否填写正确！");
			alert("资产编号第16位和第17位数字不为00，与小类号不一致，请检查资产编号是否填写正确！");
			return false;
		}
	} 
	...
	var zch = $("select[name=stationNum]").children("option:selected").attr("id").substr(2,4);		//车站号2位
	var dlh = $("select[name=type1]").children("option:selected").attr("id");		//大类号2位
	var zlh = $("select[name=type2]").children("option:selected").attr("id").substr(2,4);		//中类号2位
	var xlh;
...
function findAsset(){
	$.ajax({
			type : 'post',
			url : 'findAssetInfoByAssetIdAndXh.action?random='+Math.random(),
			dataType:'json',
			cache : false,
			data:{
...
//查询供应商
function findSupplier(object){
	var $supplier = $(object);
	if($supplier.find("option").length<=1){
		$.ajax({
			type : 'post',
			url : "<%=basePath%>/supplier/showSupplier.action?random="+Math.random(),
			dataType:'json',
			cache : false,
			data:{
				type : "1"
			...
			success:function(object){
				if(object!=null && object.length>0){
					var addHtml ; 
					for(var i=0; i<object.length; i++){
						addHtml += "<option value='"+object[i].id+"'>"+object[i].name+"</option>";
					}
					$("#supplier").append(addHtml);
				}
			}
		});
	}
...
<ul>
	<li><a href="#">资产管理</a></li>
	<li class="fin" onclick="findAsset();">资产新增</li>
</ul>
</div>
...
<div class="mb10">
<input type="hidden" id="failInfo" value="<s:property value="#request.failInfo"/>">
<s:form action="saveAdd" name="assetInfo" namespace="/asset" method="post">
	<table width="100%" class="table_1">
		...
		<tbody>
			...
			<tr>
				...
				<td id="must">
					<input maxlength="20" type="text" name="assetId" class="input_large" value="<s:property value="assetInfo.assetId"/>" />&nbsp;&nbsp;*
				</td>
				...
				<td id="must">
					<input id="hideManufacturer" name="manufacturer" type="hidden">
					<input id="showManufacturer" type="text" class="input_xxlarge">
					&nbsp;&nbsp;*
				</td>
			</tr>
			<tr>
				...
				<td id="must">
					<input id="hideVendor" name="vendor" type="hidden">
					<input id="showVendor" type="text" class="input_xxlarge">
					&nbsp;&nbsp;*
				</td>
			</tr>
			<tr>
				...
				<td id="must">
					<select name="type1" class="input_large" onchange="clearOPtions(0);chooseType(2);">
						...
						<s:iterator value="#request.type1List" id="list">
							<s:if test="#request.assetInfo.type1==#list.id">
								<option value="<s:property value="#list.id"/>" id="<s:property value="#list.code"/>" selected="selected">(<s:property value="#list.code"/>)<s:property value='#list.name'/></option>
							</s:if>
							<s:else>
								<option value="<s:property value="#list.id"/>" id="<s:property value="#list.code"/>">(<s:property value="#list.code"/>)<s:property value='#list.name'/></option>
							</s:else>
						</s:iterator>
					</select>
					&nbsp;*
				...
				<td id="must">
					<select name="type2" class="input_large" onchange="chooseType(3);clearOPtions(1);" >
						...
						<s:iterator value="#request.type2List" id="list">
							<s:if test="#request.assetInfo.type2==#list.id">
				...
				<td id="must">
					<select name="type3" class="input_large">
						<option value='-1'>---请选择---</option>
						<s:iterator value="#request.type3List" id="list">
							<s:if test="#request.assetInfo.type3==#list.id">
								<option value="<s:property value="#list.id"/>" id="<s:property value="#list.code"/>" selected="selected"><s:property value='#list.name'/></option>
							</s:if>
							<s:else>
								<option value="<s:property value="#list.id"/>" id="<s:property value="#list.code"/>"><s:property value='#list.name'/></option>
			...
			<tr>
				...
				<td id="must">
					<input id="hideUsePerson" name="usePerson" type="hidden">
					<input id="showUsePerson" type="text" id="" class="input_large" />&nbsp;&nbsp;*
				</td>
				<td class="t_r lableTd">权属责任人</td>
				<td id="must">
					<input id="hideOwner" name="owner" type="hidden">
					<input id="showOwner" type="text" class="input_large" value=""/>&nbsp;&nbsp;*
				</td>
				...
				<td id="must">
					<select name="depreciationWay" class="input_large">
						...
						<s:iterator value="#request.depreciationList" id="depreciation">
							<s:if test="#request.assetInfo.depreciationWay==#depreciation.id">
				...
				<td id="must">
					<select name="useState" class="input_large">
						...
						<s:iterator value="#request.useStatusList" id="useStatus">
							<s:if test="#request.assetInfo.useState==#useStatus.id">
				...
				<td id="must">
					<select name="equipmentState" class="input_large">
						...
						<s:iterator value="#request.equipmentStatusList" id="equipmentStatusList">
							<s:if test="#request.assetInfo.equipmentState==#equipmentStatusList.id">
				...
				<td id="must">
					<select name="mpztbw" class="input_large">
						...
						<s:iterator value="#request.namePlatePosition" id="namePlatePosition">
							<s:if test="#request.assetInfo.mpztbw==#namePlatePosition.id">
